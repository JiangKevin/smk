...
int main()
{
    ...
    int  w      = get_windows_w();
    int  h      = get_windows_h();
    auto window = smk::Window( w, h, "test" );
    window.SetShaderProgram( window.shader_program_3d() );
    window.shader_program_3d().SetUniform( "light_position", glm::vec4( 0.f, 5.f, 0.f, 1.f ) );
    ...
    window.ExecuteMainLoop(
        [ & ]
        {
            ...
            window.SetView( glm::perspective( 70.f, float( window.width() ) / window.height(), 0.1f, 100.f ) );
